...
import (
	...
	"github.com/kanopy-platform/gateway-certificate-controller/pkg/v1/cache"
	...
	v1networking "istio.io/api/networking/v1"
	...
	v1metacertmanager "github.com/cert-manager/cert-manager/pkg/apis/meta/v1"
)
...
type certificateHandler interface {
	CreateCertificate(ctx context.Context, gateway *networkingv1.Gateway, server *v1networking.Server) error
	UpdateCertificate(ctx context.Context, cert *v1certmanager.Certificate, gateway *networkingv1.Gateway, server *v1networking.Server) error
}
...
func (c *GatewayController) CreateCertificate(ctx context.Context, gateway *networkingv1.Gateway, server *v1networking.Server) error {
	log := log.FromContext(ctx)
	issuer := c.clusterIssuer

	if i, ok := gateway.Annotations[v1labels.IssuerAnnotation]; ok {
		issuer = i
	}

	if server.Tls.Mode != v1networking.ServerTLSSettings_SIMPLE {
		return nil
	}
	...
	cert := &v1certmanager.Certificate{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:      map[string]string{v1labels.ManagedLabel: fmt.Sprintf("%s.%s", gateway.Name, gateway.Namespace)},
		...
		Spec: v1certmanager.CertificateSpec{
			...
			IssuerRef: v1metacertmanager.ObjectReference{
				Kind:  "ClusterIssuer",
				Name:  issuer,
	...
	if b, ok := gateway.Annotations[v1labels.IssueTemporaryCertificateAnnotation]; ok && b == "true" {
		cert.Annotations[v1certmanager.IssueTemporaryCertificateAnnotation] = "true"
	}

	if b, ok := gateway.Annotations[v1labels.HTTPSolverAnnotation]; ok && b == "true" {
...
func (c *GatewayController) UpdateCertificate(ctx context.Context, cert *v1certmanager.Certificate, gateway *networkingv1.Gateway, server *v1networking.Server) error {
	log := log.FromContext(ctx)
...
func updateHTTPSolver(ctx context.Context, cert *v1certmanager.Certificate, gateway *networkingv1.Gateway, label string) (*v1certmanager.Certificate, bool) {
	...
	if h, ok := gateway.Annotations[v1labels.HTTPSolverAnnotation]; ok && h == "true" {
		if cert.Labels == nil {
...
func updateCertificateDNSNames(ctx context.Context, cert *v1certmanager.Certificate, server *v1networking.Server) (*v1certmanager.Certificate, bool) {
